...
def main(ms_dataset):
    ...
    if not DEBUG_CONFIGS['manual_flag']:
        logging.info("Calculating bad baselines based on angular dispersion in phases...")
        r_flagger = RFlagger(measurement_set)
        r_flagger.get_bad_baselines('flux_calibration')
        logging.info("Calculating bad baselines based closure phases...")
    ...
    logging.info("Started Detail Flagging...")
