...
from artip.casa.flag_reasons import BAD_ANTENNA_TIME, BAD_BASELINE_TIME
...
from artip.utilities.helpers import format_spw_with_channels, create_dir
from artip.utilities.log_event_handler import LogEventHandler
...
class CasaRunner:
    def __init__(self, dataset_path, output_path):
        ...
        self._casa_scripts_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/casa_scripts'
    ...
    def flagdata(self, flag_file, reasons="any"):
        ...
        script_path = '{0}/flag.py'.format(self._casa_scripts_dir)
    ...
    def quack(self):
        ...
        script_path = '{0}/quack.py'.format(self._casa_scripts_dir)
    ...
    def generate_flag_summary(self, flagging_type, scan_list, source_type="All"):
        if config.PIPELINE_CONFIGS['flag_summary']:
            script_path = '{0}/flag_summary.py'.format(self._casa_scripts_dir)
    ...
    def apply_flux_calibration(self, source_config, run_count):
        ...
        script_path = '{0}/flux_calibration.py'.format(self._casa_scripts_dir)
    ...
    def apply_bandpass_calibration(self, source_config):
        ...
        script_path = '{0}/bandpass_calibration.py'.format(self._casa_scripts_dir)
        fields = ",".join(map(str, config.GLOBAL_CONFIGS['flux_cal_fields']))
        refant = config.GLOBAL_CONFIGS['refant']
    ...
    def apply_phase_calibration(self, flux_cal_field, source_config):
        ...
        script_path = '{0}/phase_calibration.py'.format(self._casa_scripts_dir)
    ...
    def apply_target_source_calibration(self, source_id):
        ...
        script_path = '{0}/target_source_calibration.py'.format(self._casa_scripts_dir)
    ...
    def r_flag(self, source_type, source_ids):
        script_path = '{0}/r_flag.py'.format(self._casa_scripts_dir)
    ...
    def tfcrop(self, source_type, source_ids):
        script_path = '{0}/tfcrop.py'.format(self._casa_scripts_dir)
        source_ids = ','.join([str(source_id) for source_id in source_ids])
    ...
    def setjy(self, source_id, source_name):
        ...
        script_path = '{0}/setjy.py'.format(self._casa_scripts_dir)
        freq_band = "L"
    ...
    def split(self, output_path, filters):
        script_path = '{0}/split_dataset.py'.format(self._casa_scripts_dir)
    ...
    def base_image(self):
        ...
        script_path = '{0}/base_image.py'.format(self._casa_scripts_dir)
    ...
    def apply_self_calibration(self, selfcal_config, calibration_mode, output_ms_path, output_path, spw):
        ...
        script_path = '{0}/self_calibration.py'.format(self._casa_scripts_dir)
    ...
    def fourier_transform(self, field_name, cal_mode, spw_range, loop_count):
        ...
        script_path = '{0}/fourier_transform.py'.format(self._casa_scripts_dir)
    ...
    def apply_line_calibration(self, calmode_config, parent_source_id, mode):
        ...
        script_path = '{0}/apply_line_calibration.py'.format(self._casa_scripts_dir)
    ...
    def create_line_image(self, calmode_config, parent_source_id):
        ...
        script_path = '{0}/create_line_image.py'.format(self._casa_scripts_dir)
        cont_mode = 'ref'
