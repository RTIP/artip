...
class PhaseSetTest(TestCase):
    @classmethod
    def setUpClass(self):
        self.r_threshold = Config('conf/config.yml').get('flux_calibration')['r_threshold']

    def test_is_dispersed_should_return_false_for_dispersion_of_less_than_200_degrees_for_flux_calibrator(self):
        phase_data_in_radian = self.degree_to_radian([41,50,50,29,13,10,14,12,22,47,31,34,43,40,40,42,15,13,41,24,13,22,28,47,37])
    ...
    def test_is_dispersed_should_return_true_for_dispersion_more_than_270_degrees(self):
        phase_data_in_radian = self.degree_to_radian([19,28,8,200,20,190,92,15,145,118,211,266,189,89,56,52,178,162,149,169,219,26,270,235,67])
        phase_set = PhaseSet(phase_data_in_radian)
